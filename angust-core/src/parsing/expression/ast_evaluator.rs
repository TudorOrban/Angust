...
use crate::rendering::elements::component::{component_state::{ReactiveState, StateValue}, functions::component_functions::ComponentFunctions};
...
//             match state.get_property(&[name]) {
//                 Some(StateValue::Text(text)) => Ok(Box::new(text)),
//                 Some(StateValue::Number(num)) => Ok(Box::new(num)),
//                 Some(StateValue::Boolean(boolean)) => Ok(Box::new(boolean)),
//                 Some(StateValue::Nested(_)) => {
//                     // Handle if you expect to use nested states in AST evaluation
//                     Err(format!("Nested state found for '{}', but not supported in AST evaluation", name))
//                 },
//                 None => Err(format!("Property {} not found in component state", name)),
//             }
//         },
